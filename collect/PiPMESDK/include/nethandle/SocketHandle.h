...
public:
     ...
     */
    bool operator==(const SockAddrIn& sin) const {
		return IsEqual(sin);
	}
     ...
     */
    void SetAddr(const sockaddr_in* psin) {
		SetAddr(reinterpret_cast<const sockaddr_storage*>(psin));
	}
     ...
     */
    void SetAddr(const sockaddr_in6* psin) {
		SetAddr(reinterpret_cast<const sockaddr_storage*>(psin));
	}
     ...
     */
    void SetAddr(const sockaddr_storage* pss) {
		ss_family = pss->ss_family; memcpy(this, pss, Size());
	}
...
public:
	...
	/**
     * Get IPv4 address of a host
     * @param arrIPAddr contains IPv4 local address
     * @return void
     */
	static void GetLocalIPv4Address(LPCTSTR pszHostName, CStringArray& arrIPAddr);

	/**
     * Get IPv4 address subnet mask
     * @param arrIPAddr contains IPv4 address
	 * @param strSubnetMask retrieve subnet mask
     * @return true if successful
     */
	static ULONG GetSubnetMask(CString strIPv4Addr);

	/**
     * Get IPv6 address of a host
     * @param arrIPAddr contains IPv6 link local address
     * @return true if successful
     */
	static bool CSocketHandle::GetLocalIPv6Address(CStringArray& arrIPAddr);

	static vector<LocalNetworkInfo> CSocketHandle::GetLocalIPv4NetworkInfo();
