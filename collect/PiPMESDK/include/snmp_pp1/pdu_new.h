...
class DLLOPT Pdu
{
 public:
  ...
  /**
   * Add from_address in printable format
   */
  int add_from_addr(char* pip);

  /**
   * Get from_address in printable format, [][128];
   */
  char* get_from_addr(int* ncount);

  /**
   * Delete one of from_address in printable format
   */
  bool del_from_addr(char* pip);

  /**
   * Delete all of from_address
   */
  void del_all_from_addr();
 ...
 protected:
  ...
  char ip_from_addr_list[MAXADDR][MAXIP];
  int ip_ncount;
#ifdef _SNMPv3
